...
CREATE TABLE bind_views ( view_id INT NOT NULL AUTO_INCREMENT, org_id INT NOT NULL, view_name VARCHAR(40) NOT NULL, def_sh_id INT NOT NULL, rpz_fqdn VARCHAR(16) NOT NULL, description VARCHAR(80), PRIMARY KEY (view_ID), FOREIGN KEY (org_id) REFERENCES org_info(org_id) ON UPDATE CASCADE ON DELETE NO ACTION, FOREIGN KEY (def_sh_id) REFERENCES view_sinkholes(sinkhole_id) ON UPDATE CASCADE ON DELETE NO ACTION ) ENGINE=INNODB;
