...
CREATE TABLE bind_views ( view_id INT NOT NULL AUTO_INCREMENT, org_id INT NOT NULL, view_name VARCHAR(40) NOT NULL, def_sh_id INT NOT NULL, view_src_acl_ips VARCHAR(100) NOT NULL, description VARCHAR(80),tsig_id INT NOT NULL, PRIMARY KEY (view_ID), FOREIGN KEY (org_id) REFERENCES org_info(org_id) ON UPDATE CASCADE ON DELETE NO ACTION, FOREIGN KEY (def_sh_id) REFERENCES view_sinkholes(sinkhole_id) ON UPDATE CASCADE ON DELETE NO ACTION, FOREIGN KEY (tsig_id) REFERENCES tsig_keys(tsig_id) ON UPDATE CASCADE ON DELETE NO ACTION ) ENGINE=INNODB;
